#include<bits/stdc++.h>
using namespace std; 
vector<int> parent(100); vector<int> sz(100); void make_set(int v)
{
parent[v] = v; sz[v] = 1;
}
int find_set(int v)
...
{
a = find_set(a); b = find_set(b); if (a != b)
...
vector<vector<int>> graph; for (int i = 0; i < e; i++)
{
int u, v, w;
cin >> u >> v >> w; graph.push_back({w, u, v});
...
{
total_weight += w; union_set(u, v); //add to set
}
}
